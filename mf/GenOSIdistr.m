function varargout = GenOSIdistr(N, maxMoments, nDeltas, nThetas, ...
                                 kappa, mExtZero, mExtOne, mE1Guess, ...
                                 Jab, Ea, cff)

    if N >= 1000000
        [tcE, mE1_sol, tcI, mI1_sol, corr, thetas, phiFF] = GenTuningCurves_parfor(N, ...
                                                      maxMoments, ...
    ...
    filename = ['./data/OSI/osi_mZero_', num2str(mExtZero * 1e3), ...
               '_mOne_', num2str(1e4*mExtOne), '_kappa_', num2str(kappa), ...
                '_N', num2str(N)]
    save(filename, 'osi_E', 'osi_I')

    filename = ['./data/TC/tc_mZero_', num2str(mExtZero * 1e3), ...
               '_mOne_', num2str(1e4*mExtOne), '_kappa_', num2str(kappa), '_N', num2str(N)]
    save(filename, 'tcE', 'tcI')
    
    save(['./data/OSI/osi_mZero', num2str(mExtZero), '_mOne_', ...
         num2str(1e6*mExtOne), '_kappa_', num2str(kappa), '_N', num2str(N)], 'saveOut')    

    tc.E = tcE;
    tc.I = tcI;                                                                     
    save(['./data/TC/tc_kappa_', num2str(10*kappa), '_mExtZero_', ...
          num2str(1e6*mExtZero), '_mOne_', num2str(1e4*mExtOne), '_N', num2str(N)], 'tc')    
